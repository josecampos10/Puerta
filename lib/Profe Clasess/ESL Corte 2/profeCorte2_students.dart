...
class ProfeCorte2students extends StatefulWidget {
  const ProfeCorte2students({super.key});
  @override
  State<ProfeCorte2students> createState() => _ProfeCorte2studentsState();
}

class _ProfeCorte2studentsState extends State<ProfeCorte2students> {
  final currentUser = FirebaseAuth.instance.currentUser!;
  CollectionReference users = FirebaseFirestore.instance.collection('postsCorte2');
  ...
  Future uploadFile() async {
    final path = 'Corte2files/${pickedFile!.name}';
  ...
  void initState() {
    super.initState();
    futureFiles = FirebaseStorage.instance.ref('/Corte2files').listAll();
    getProfilePicture();
    futureUserDoc =
        FirebaseFirestore.instance.collection('clases').doc('Corte y Confección 2').get();
    futureUsers = FirebaseFirestore.instance
        .collection('users')
        .orderBy('name', descending: false)
        .get();
...
final filtered = snap.where((d) {
  ...
  final ciudadania = data['Corte2']?.toString().trim();
